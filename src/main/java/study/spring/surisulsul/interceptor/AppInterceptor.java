package study.spring.surisulsul.interceptor;
...
public class AppInterceptor implements HandlerInterceptor {
	 ...
	 * boolean 값을 전달 : false인 경우 controller 실행하지 안고 요청 종료 보통 이곳에서 각종 체크작업과 로그를 기록하는
	 * 작업 진행
	 */
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 이전 종료 시간이 0보다 크다면 새로운 시작시간과의 차이는 이전 페이지에 머문시간을 의미
	...
	/**
	 * view 단으로 forward 되기 전에 수행 컨트롤러 로직이 실행된 이후 호출 컨트롤러 단에서 에러 발생 시 해당 메서드는 수행 X
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		// 시작 시간과 종료시간 사이의 차이를 구하면 페이지 실행시간을 구할 수 있다.
