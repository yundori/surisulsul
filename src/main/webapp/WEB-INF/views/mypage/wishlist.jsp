	...
	<ul class="psn_result_inner swiper-wrapper">
		...
		<c:forEach var="item" items="${member_product }" varStatus="status">
			<li class="pick_item swiper-slide">
				<%--상세 페이지로 이동하는 URL --%> <c:url value="/item_details.do"
					var="viewUrl">
					<c:param name="prodid" value="${item.id}" />
				</c:url> <a href="${viewUrl}" class="pick_link"> <span
					class="psn_pick_bg"> <img class="psn_pick"
						src="${contextPath}/assets/img/items/${item.img}">
				</span> <span class="psn_pick_text">
...
<c:otherwise>
	...
	<div class="psn_no_result">
		<img
			src="${pageContext.request.contextPath}/assets/img/mark_alert.png" />
		<h3 class="psn_title_dsc">
			아직 관심있어요를 누른 상품이 없습니다. <br /> <span class="psn_gotest">관심있는
				상품을 찜해두면 관련 할인 정보를 빠르게 확인할 수 있습니다.</span>
		</h3>
		<a href="${pageContext.request.contextPath}/item_filtered.do"
			class="btn_blue">관심있는 상품 담으러 가기</a>
	</div>
