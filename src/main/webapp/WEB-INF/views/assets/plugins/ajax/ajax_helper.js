...
$(function() {
	...
	$.ajaxSetup({
		...
		// 통신 시작 전에 실행할 기능 (ex. 로딩바 표시)
		beforeSend: function() {
			// 현재 통신 중인 대상 페이지를 로그로 출력
			console.log(">> Ajax 통신 시작 >> " + this.url);
			loader.show();
		},

		// 통신 실패 시 호출될 함수 (파라미터는 에러 내용)
		error: function(error) {
			// 404 -> Page Not Found
			// 50x -> Server Error (웹 프로그램 에러)
			// 200, 0 -> 내용의 형식 에러 (JSON, XML)
			console.log(">> 통신 실패 >> " + error.status);
		...
		// 성공, 실패에 관계없이 맨 마지막에 무조건 호출됨 (ex. 로딩바 닫기)
		complete: function() {
